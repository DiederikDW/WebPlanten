...
3. Installeer de backend pakketten:

   navigeer naar je backend in je terminal met commando
   
   cd backend

	installeer de pakketten met commando

   npm install
   
5. Installeer de frontend pakketten:
...
1. Start de backend server(express) in je eerste terminal venster
   ...
   als het gelukt is zie je in de terminaal de boodschap:
   
   Plantenserver draait op http://localhost:3000
   
   laat dit terminal venster open
  
3. Start de frontend in je tweede terminal venster
...
In je terminal verschijnt nu een link (meestal http://localhost:5173) waar je de website zal terugvinden
